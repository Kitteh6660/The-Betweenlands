...
@OnlyIn(Dist.CLIENT)
public class RenderRuneCarvingTableFiller extends TileEntityRenderer<TileEntityRuneCarvingTableFiller> {
	...
	public void render(TileEntityRuneCarvingTableFiller tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		if(tile != null && tile.hasWorld() && destroyStage >= 0) {
			...
			int rotation = StatePropertyHelper.getStatePropertySafely(tile, BlockRuneCarvingTable.class, BlockRuneCarvingTable.FACING, Direction.NORTH).getHorizontalIndex() * 90;
