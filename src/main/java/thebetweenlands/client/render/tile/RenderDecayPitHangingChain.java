...
@OnlyIn(Dist.CLIENT)
public class RenderDecayPitHangingChain extends TileEntityRenderer<TileEntityDecayPitHangingChain > {
	...
	public void render(TileEntityDecayPitHangingChain tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
		...
		for (int len = 0; len <= 4 + MathHelper.floor(tile.getProgress()* 0.0078125F); len++) {
	...
	private void renderChainpart(TileEntityDecayPitHangingChain tile, double x, double y, double z, float scroll, float angle) {
		...
		GlStateManager.translate(x, y -2.5D, z);
	...
	@Override
	public boolean shouldRenderOffScreen(TileEntityDecayPitHangingChain tile) {
		return true;
	}
}