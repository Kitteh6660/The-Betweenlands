...
public class CenserRenderer extends TileEntityRenderer<TileEntityCenser> {
	
	private final ModelCenser MODEL = new ModelCenser();
	...
	public CenserRenderer(TileEntityRendererDispatcher dispatcher) {
		super(dispatcher);
	}
	
	@Override
	public boolean shouldRenderOffScreen(TileEntityCenser te) {
		return true;
	}
	...
	private void renderFogFlow(TextureAtlasSprite fogSprite, float animationTicks, int skyLight, int blockLight, int color, float alpha, boolean flatSlope) {
		...
		float minU = fogSprite.getU0();
		float minV = fogSprite.getV0();
		float maxU = fogSprite.getU1();
		float maxV = fogSprite.getV1();
		...
		for(int i = 0; i < 16; i++) {
			...
			buffer.vertex(-halfWidth, prevStripY, prevZOff).color(r, g, b, a).tex(minU, vOff + oneTx).lightmap(skyLight, blockLight).endVertex();
			buffer.vertex(-halfWidth, stripY, wavyZOff).color(r, g, b, a).tex(minU, vOff).lightmap(skyLight, blockLight).endVertex();
			buffer.vertex(halfWidth, stripY, wavyZOff).color(r, g, b, a).tex(maxU, vOff).lightmap(skyLight, blockLight).endVertex();
			buffer.vertex(halfWidth, prevStripY, prevZOff).color(r, g, b, a).tex(maxU, vOff + oneTx).lightmap(skyLight, blockLight).endVertex();
	...
	@Override
	public void render(TileEntityCenser te, float p_225616_2_, MatrixStack matrix, IRenderTypeBuffer buffer, int p_225616_5_, int partialTicks) {
		BlockState blockstate = te.getBlockState();
		
		//matrix.enableBlend();
		//matrix.blendFunc(matrix.SourceFactor.SRC_ALPHA, matrix.DestFactor.ONE_MINUS_SRC_ALPHA);
		//matrix.color(1.0F, 1.0F, 1.0F, 1.0F);

		bindTexture(TEXTURE);

		matrix.pushPose();
		matrix.translate(x + 0.5F, y + 1.5F, z + 0.5F);
		matrix.scale(1F, -1F, -1F);
		matrix.rotate(StatePropertyHelper.getStatePropertySafely(te, BlockCenser.class, BlockCenser.FACING, Direction.NORTH).getHorizontalAngle(), 0.0F, 1F, 0F);
		matrix.disableCull();

		RenderMaterial rendermaterial = getMaterial(blockstate.getBlock());
		IVertexBuilder ivertexbuilder = rendermaterial.buffer(buffer, this.MODEL::renderType);
		...
		if(te != null && te.getFuelTicks() > 0) {
			...
			if(recipe != null) {
				if (te.hasLevel()) {
					...
					matrix.depthMask(false);

					matrix.pushPose();
					matrix.translate(x + 0.5F, y + 0.7F, z + 0.5F);

					matrix.rotate(-StatePropertyHelper.getStatePropertySafely(te, BlockCenser.class, BlockCenser.FACING, Direction.NORTH).getHorizontalAngle() - 90, 0.0F, 1F, 0F);
					...
					//top fog strips
					matrix.pushPose();
					matrix.translate(0.0f, 0.18f, 0.18f);
					...
					matrix.pushPose();
					matrix.translate(0.03f, 0.18f, 0.15f);
					matrix.mulPose(Vector3f.YP.rotationDegrees(90)); // .rotate(90, 0, 1, 0);
					...
					//bottom fog strips
					matrix.pushPose();
					matrix.translate(0.114f, 0, 0.235f);
					...
					matrix.pushPose();
					matrix.translate(-0.135f, 0, 0.235f);
