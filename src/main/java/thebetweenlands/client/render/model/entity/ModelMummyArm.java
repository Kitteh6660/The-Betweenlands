...
public class ModelMummyArm extends MowzieModelBase {
	...
	public void renderToBuffer(MatrixStack pMatrixStack, IVertexBuilder pBuffer, int pPackedLight, int pPackedOverlay, float pRed, float pGreen, float pBlue, float pAlpha) { 
		...
		this.armleft.render(pMatrixStack, pBuffer, pPackedLight, pPackedOverlay, pRed, pGreen, pBlue, pAlpha);
	}
	...
	@Override
	public void setLivingAnimations(LivingEntity entity, float yaw, float pitch, float partialTicks) {
		setToInitPose();

		EntityMummyArm arm = (EntityMummyArm) entity;

		armleft.yRot = (float) Math.toRadians(yaw);
		armleft.xRot = 3.4416F;
		armleft.zRot = -0.1F;
		float offset = 0.0F;
		...
		swing(armleft, 1, 0.4f, false, offset, 0, (arm.tickCount + partialTicks) / 10.0F, 1);
		walk(armleft2, 1, 0.4f, false, offset, 0, (arm.tickCount + partialTicks) / 8.0F, 1);
		swing(armleft2, 1, 0.1f, false, offset, 0, (arm.tickCount + partialTicks) / 4.0F, 1);
		walk(armleft, 1, 0.1F, false, offset, 0, (arm.tickCount + partialTicks) / 10.0F, 1);
		walk(armleft, 2, 0.3F, false, offset, 0, (arm.tickCount + partialTicks) / 15.0F, 1);
		bob(armleft, 2, 1.8F, false, (arm.tickCount + partialTicks) / 15.0F, 1);
	}
}
