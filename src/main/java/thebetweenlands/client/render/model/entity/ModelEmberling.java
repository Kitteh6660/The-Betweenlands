...
@OnlyIn(Dist.CLIENT)
public class ModelEmberling extends Model {
    ModelRenderer body3;
    ModelRenderer body4;
	...
	public void setLivingAnimations(LivingEntity entity, float limbSwing, float limbSwingAngle, float partialRenderTicks) {
		...
		if(emberling.getIsFlameAttacking()) {
			...
			jaw_lower.xRot = (!emberling.isSitting() ? 0.40980330836826856F : 0.2F ) + flap * (!emberling.isSitting() ? 0.5F : 0.125F);
			head_main.zRot = 0F;
		}
			
        if (emberling.isSitting()) {
        	body3.rotationPointY= 20.5F;
			hindleg_left1.xRot = -1.4F;
			hindleg_right1.xRot = -1.4F;
	
			hindleg_left2.xRot = 0.7740535232594852F;
			hindleg_right2.xRot = 0.7740535232594852F;
	
			arm_right1.xRot = -1.1F;
			arm_right1.yRot = 0.091106186954104F;
			arm_right1.zRot = 0.18203784098300857F;
	
			arm_left1.xRot = -1.1F;
			arm_left1.yRot = -0.091106186954104F;
			arm_left1.zRot = -0.18203784098300857F;
	
			body2.yRot = 0F;
			body4.yRot = 0F ;
			
			body2.xRot = 0F + flap * 0.0125F;
			body4.xRot = -0.18203784098300857F - flap * 0.025F;
