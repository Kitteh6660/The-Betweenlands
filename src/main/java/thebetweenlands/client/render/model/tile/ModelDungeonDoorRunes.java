...
@OnlyIn(Dist.CLIENT)
public class ModelDungeonDoorRunes extends Model {
	...
	public void render(TileEntityDungeonDoorRunes tile, float scale, float partialTicks) {
		top.xRot = 0F + (tile.lastTickTopRotate + (tile.top_rotate - tile.lastTickTopRotate) * partialTicks) / (180F / (float) Math.PI);
		mid.xRot = 0F + (tile.lastTickMidRotate + (tile.mid_rotate - tile.lastTickMidRotate) * partialTicks) / (180F / (float) Math.PI);
		bottom.xRot = 0F + (tile.lastTickBottomRotate + (tile.bottom_rotate - tile.lastTickBottomRotate) * partialTicks) / (180F / (float) Math.PI);

		if(tile.isMimic()) {
			slate1.xRot = 0F + (tile.last_tick_slate_1_rotate + (tile.slate_1_rotate - tile.last_tick_slate_1_rotate) * partialTicks) / (180F / (float) Math.PI);
			slate2.xRot = 0F + (tile.last_tick_slate_2_rotate + (tile.slate_2_rotate - tile.last_tick_slate_2_rotate) * partialTicks) / (180F / (float) Math.PI);
			slate3.xRot = 0F + (tile.last_tick_slate_3_rotate + (tile.slate_3_rotate - tile.last_tick_slate_3_rotate) * partialTicks) / (180F / (float) Math.PI);
		}
		if(!tile.isMimic()) {
			slate1.xRot = 0F;
			slate2.xRot = 0F;
