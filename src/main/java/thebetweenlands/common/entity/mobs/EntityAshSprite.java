...
import thebetweenlands.common.entity.mobs.EntityWallFace.MoveHelper;
...
public class EntityAshSprite extends MobEntity implements IEntityBL {
	...
	public EntityAshSprite(World worldIn) {
		super(worldIn);
		fireImmune = true;
		moveControl = new EntityAshSprite.AIMoveControl(this);
		setSize(0.4F, 0.8F);
		experienceValue = 3;
	}
	...
	public void tick() {
		...
		setNoGravity(true);
		if (level.isClientSide())
			addParticles(level, getPosition(), rand);
	}
	...
	@OnlyIn(Dist.CLIENT)
	public void addParticles(World worldIn, BlockPos pos, Random rand) {
		for(int i = 0; i < 4; i++) {
			worldIn.addParticle(ParticleTypes.SUSPENDED_DEPTH, pos.getX() + 0.5D + (rand.nextBoolean() ? -0.5F : 0.5F) * Math.pow(rand.nextFloat(), 1F), pos.getY() + 0.5D + rand.nextFloat() - 0.5F, pos.getZ() + 0.5D + (rand.nextBoolean() ? -0.5F : 0.5F) * Math.pow(rand.nextFloat(), 1F), 0, 0.2D, 0);
		}
	}
	...
	protected void registerGoals() {
		...
		tasks.addGoal(4, new EntityAshSprite.AIChargeAttack(this));
		tasks.addGoal(8, new EntityAshSprite.AIMoveRandom(this));
		tasks.addGoal(9, new EntityAIWatchClosest(this, PlayerEntity.class, 3.0F, 1.0F));
		tasks.addGoal(10, new EntityAIWatchClosest(this, MobEntity.class, 8.0F));
		targetTasks.addGoal(1, new EntityAIHurtByTarget(this, true, new Class[] { EntityAshSprite.class }));
		targetTasks.addGoal(3, new EntityAINearestAttackableTarget<>(this, PlayerEntity.class, false));
	}
	...
	private boolean getAshSpriteFlag(int mask) {
		int i = ((Byte) entityData.get(ASH_SPRITE_FLAGS)).byteValue();
		return (i & mask) != 0;
	}
	...
	private void setAshSpriteFlag(int mask, boolean value) {
		...
		if (value)
			...
			i = i & ~mask;
		entityData.set(ASH_SPRITE_FLAGS, Byte.valueOf((byte) (i & 255)));
	}
	...
	class AIChargeAttack extends EntityAIBase {
		...
		public void start() {
			...
			if(target != null) {
				...
				ashSprite.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1.0D);
		...
		public void updateTask() {
			...
			for (int i = 0; i < 3; ++i) {
				...
				if (ashSprite.world.isEmptyBlock(blockpos1)) {
					ashSprite.moveControl.setWantedPosition((double) blockpos1.getX() + 0.5D, (double) blockpos1.getY() + 0.5D, (double) blockpos1.getZ() + 0.5D, 0.25D);
