...
public abstract class EntityMovingWallFace extends EntityWallFace {
	...
	public static final Attribute BLOCK_TRACKING_SEARCH_RANGE = (new RangedAttribute((Attribute)null, "bl.blockTrackingSearchRange", 8.0D, 0.0D, 32.0D)).setDescription("Block Tracking Search Range");
	public static final Attribute BLOCK_FIX_SEARCH_RANGE = (new RangedAttribute((Attribute)null, "bl.blockFixSearchRange", 8.0D, 0.0D, 32.0D)).setDescription("Block Fix Search Range");
	...
	public void tick() {
		...
		if(this.isMoving() && this.level.isClientSide()) {
			if(this.tickCount % 3 == 0) {
				...
				for(int i = 0; i < 24; i++) {
					double rx = (this.level.random.nextDouble() - 0.5D) * this.width;
					double ry = (this.level.random.nextDouble() - 0.5D) * this.height;
					double rz = (this.level.random.nextDouble() - 0.5D) * this.width;
					BlockPos pos = new BlockPos(px + rx, py + ry, pz + rz);
					BlockState state = this.level.getBlockState(pos);
					if(!state.getBlock().isAir(state, this.level, pos)) {
						double mx = facing.getStepX() * 0.15F + (this.level.random.nextDouble() - 0.5D) * 0.25F;
						double my = facing.getStepY() * 0.15F + (this.level.random.nextDouble() - 0.5D) * 0.25F;
						double mz = facing.getStepZ() * 0.15F + (this.level.random.nextDouble() - 0.5D) * 0.25F;
						this.level.addParticle(ParticleTypes.BLOCK_DUST, px + rx, py + ry, pz + rz, mx, my, mz, Block.getStateId(state));
					}
				}
			}
		}
	...
	public boolean canMoveFaceInto(BlockPos pos, Direction facing, Direction facingUp) {
		BlockState state = this.level.getBlockState(pos);
		if(state.getMaterial().isLiquid() || state.getBlock().isAir(state, this.level, pos)) {
			return true;
		}
		if(state.getBlock().isLeaves(state, this.level, pos)) {
			return false;
		}
		List<AxisAlignedBB> collisionBoxes = new ArrayList<>();
		state.addCollisionBoxToList(this.level, pos, new AxisAlignedBB(pos), collisionBoxes, this, false);
	...
	protected void fixUnsuitablePosition(int violatedChecks) {
		if(this.tickCount % 3 == 0) {
			int searchRange = MathHelper.ceil(this.getAttribute(BLOCK_FIX_SEARCH_RANGE).getValue());

			for(int i = 0; i < 50; i++) {
				float rx = this.level.random.nextFloat() * 2 - 1;
				float ry = this.level.random.nextFloat() * 2 - 1;
				float rz = this.level.random.nextFloat() * 2 - 1;
				BlockPos rndPos = new BlockPos(this.getX() + this.level.random.nextInt(searchRange * 2) - searchRange, this.getY() + this.height / 2 + this.level.random.nextInt(searchRange * 2) - searchRange, this.getZ() + this.level.random.nextInt(searchRange * 2) - searchRange);
				if(this.level.isBlockLoaded(rndPos)) {
	...
	public List<BlockPos> findNearbyBlocksForMovement() {
		final int radius = MathHelper.ceil(this.getAttribute(BLOCK_TRACKING_SEARCH_RANGE).getValue());
	...
	protected static abstract class AITrackTarget<T extends EntityMovingWallFace> extends EntityAIBase {
		...
		protected boolean isTargetVisibleAndInRange() {
			return this.entity.getSensing().canSee(this.entity.getAttackTarget()) && (!this.stayInRange || this.entity.getAttackTarget().getDistanceSq(this.entity) <= this.maxRangeSq);
		}
		...
		public void start() {
			...
			this.findBlocksCooldown = 20 + this.entity.random.nextInt(30);
		...
		public void updateTask() {
			...
			if(target != null && this.canMove()) {
				if(this.findBlocksCooldown <= 0 && (this.trackingBlocks == null || this.trackingBlocks.isEmpty())) {
					this.findBlocksCooldown = 20 + this.entity.random.nextInt(40);
				...
				if(this.trackingBlocks != null && !this.trackingBlocks.isEmpty() && this.checkCooldown <= 0) {
					this.checkCooldown = 5 + this.entity.random.nextInt(15);
					...
					for(int i = 0; i < 16; i++) {
						...
						BlockPos pos = this.trackingBlocks.remove(this.entity.random.nextInt(this.trackingBlocks.size()));
						...
						if(!this.stayInRange || target.getDistanceSqToCenter(pos) <= this.maxRangeSq) {
							...
							Vector3d lookDir = target.getDeltaMovement().add(0, target.getEyeHeight(), 0).subtract(center);
							...
							if(this.canSeeFrom(pos, facing, target) && this.entity.checkAnchorAt(center, lookDir, AnchorChecks.ALL) == 0) {
								this.entity.moveControl.setWantedPosition(center.x, center.y, center.z, 1);
	...
	protected static abstract class AIWander<T extends EntityMovingWallFace> extends EntityAIBase {
		...
		@Override
		public boolean canUse() {
			return this.canMove() && !this.entity.isMoving() && this.entity.getIdleTime() < 100 && this.entity.getRandom().nextInt(this.chance) == 0;
		}
	...
	protected static class AIAttackMelee extends EntityAIAttackMelee {
		...
		protected double getAttackReachSqr(LivingEntity attackTarget) {
			float r1 = Math.max(attackTarget.getBbWidth(), attackTarget.getBbHeight());
