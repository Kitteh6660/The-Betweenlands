...
public class ObstructionAwarePathNavigateGround<T extends MobEntity & IPathObstructionAwareEntity> extends PathNavigateGround {
	...
	protected void checkForStuck(Vector3d entityPos) {
		...
		if(this.checkObstructions && this.currentPath != null && !this.currentPath.isFinished()) {
			...
			Vector3d facingDiff = checkPos.subtract(entityPos.add(0, axis == 1 ? this.entity.height / 2 : 0, 0));
			...
			loop: for(int yo = 0; yo < height; yo++) {
				for(int xzo = -ceilHalfWidth; xzo <= ceilHalfWidth; xzo++) {
					...
					if(nodeType == PathNodeType.BLOCKED) {
						...
						if(collisionBox != null && collisionBox.offset(pos).intersects(this.obstructionAwareEntity.getBoundingBox().expand(Math.signum(diff.x) * 0.2D, Math.signum(diff.y) * 0.2D, Math.signum(diff.z) * 0.2D))) {
							blocked = true;
							break loop;
						}
					}
