...
public class FlyingNodeProcessorBL extends NodeProcessor {
	...
	public int findPathOptions(PathPoint[] pathOptions, PathPoint currentPoint, PathPoint targetPoint, float maxDistance) {
		...
		for (Direction Direction : Direction.VALUES) {
			PathPoint pathpoint = this.getAirNode(currentPoint.x + Direction.getStepX(), currentPoint.y + Direction.getStepY(), currentPoint.z + Direction.getStepZ(), targetPoint);
		...
		for (int i = x; i < x + this.entitySizeX; ++i) {
			for (int j = y; j < y + this.entitySizeY; ++j) {
				for (int k = z; k < z + this.entitySizeZ; ++k) {
					...
					BlockState state = this.blockaccess.getBlockState(checkPos);
