...
public class PlayerPortalHandler {
	...
	public static void teleportCheck(PlayerTickEvent event) {
		if(event.phase == TickEvent.Phase.END) {
			...
			if (cap != null) {
				if(cap.isInPortal()){
					BlockPos pos = new BlockPos(player.getX(), player.getY() + 0.5D, player.getZ());
					...
					if(inPortalBlock) {
						if(!cap.wasTeleported()) {
							if (cap.getTicksUntilTeleport() <= 0 || player.isCreative()) {
								if(player.world instanceof ServerWorld) {
									...
									if(targetDim != player.dimension) {
										ServerWorld otherDim = ((ServerWorld) player.world).getMinecraftServer().getWorld(targetDim);
										if(otherDim != null) {
