...
public class ManualWidgetBase {
    ...
    public static String processTimeString = I18n.get("manual.widget.process.time");
    public static String processTimeMinutesString = I18n.get("manual.widget.process.time.minutes");
    public static String processTimeSecondsString = I18n.get("manual.widget.process.time.seconds");
    public static String burnTimeString = I18n.get("manual.widget.burn.time");
     ...
     */
    @OnlyIn(Dist.CLIENT)
    public void ItemRenderer(int xPos, int yPos, ItemStack stack, boolean hasSpecialTooltip, boolean addPageLink, Item book) {
        ...
        if (!hasSpecialTooltip && mouseX >= xPos && mouseY >= yPos && mouseX <= xPos + 16 && mouseY <= yPos + 16) {
            if (!stack.isEmpty()) {
                ...
                if (addPageLink && shouldShowTooltip)
                    tooltipData.add(I18n.canTranslate("manual.open_entry") ? I18n.get("manual.open_entry"): I18n.translateToFallback("manual.open_entry"));
