...
public class TileEntityBeamOrigin extends TileEntity implements ITickable {
		...
		if (getWorld().getGameTime() % 10 == 0) {
			if (checkForLitBrazier(getPos().add(3, -1, 3))) {
				...
				if (!world.isClientSide())
					if (!beam_1_active) {
						setBeam1Active(true);
						getWorld().playSound((PlayerEntity) null, getPos().add(3, -1, 3), SoundRegistry.PORTAL_ACTIVATE, SoundCategory.BLOCKS, 0.125F, 0.25F);
					}
			} else {
				if (!world.isClientSide())
					if (beam_1_active)
	...
	public boolean checkForLitBrazier(BlockPos targetPos) {
		BlockState flame = getWorld().getBlockState(targetPos);
	...
	public void activateBlock() {
		if(!world.isClientSide()) {
			...
			if(!state.getValue(BlockBeamOrigin.POWERED)) {
				getWorld().setBlockState(getPos(), BlockRegistry.MUD_TOWER_BEAM_ORIGIN.defaultBlockState().setValue(BlockBeamOrigin.POWERED, true));
				getWorld().playSound((PlayerEntity)null, getPos(), SoundRegistry.BEAM_ACTIVATE, SoundCategory.BLOCKS, 1F, 1F);
			}
		}
	...
	public void deactivateBlock() {
		BlockState state = getWorld().getBlockState(getPos());
		if (state.getValue(BlockBeamOrigin.POWERED)) {
			getWorld().setBlockState(getPos(), BlockRegistry.MUD_TOWER_BEAM_ORIGIN.defaultBlockState().setValue(BlockBeamOrigin.POWERED, false));
