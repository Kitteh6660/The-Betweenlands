...
public class WallDampTorchBlock extends WallTorchBlock {
	...
	public WallDampTorchBlock(Properties properties, IParticleData flameParticle) {
	...
	public void animateTick(BlockState stateIn, World worldIn, BlockPos pos, Random rand) {
		if(rand.nextInt(4) == 0) {
			...
			if (facing.getAxis().isHorizontal()) {
				...
				BLParticles.SMOKE.spawn(worldIn, x + 0.27D * (double)facingOpposite.getStepX(), y + 0.22D, z + 0.27D * (double)facingOpposite.getStepZ(), ParticleArgs.get().withColor(0.2F, 0.2F, 0.2F, 1.0F));
			} else {
