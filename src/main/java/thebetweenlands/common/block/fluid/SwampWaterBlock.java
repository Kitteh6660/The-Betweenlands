...
public class SwampWaterBlock extends FlowingFluidBlock/* implements IStateMappedBlock, ITintedBlock, ICustomItemBlock*/ {
	...
	public SwampWaterBlock(Properties properties) {
		super(() -> FluidRegistry.SWAMP_WATER.get(), properties);
	}

	public SwampWaterBlock setUnderwaterBlock(boolean underwaterBlock) {
	...
	public boolean canDisplace(IBlockReader world, BlockPos pos) {
		...
		if (state.getBlock() instanceof SwampWaterBlock) {
			return false;
		}
	...
	public boolean displaceIfPossible(World world, BlockPos pos) {
		...
		if (block instanceof SwampWaterBlock) {
			return false;
		}
	...
	public float getFluidHeightForRender(IBlockReader world, BlockPos pos, @Nonnull BlockState up) {
		...
		if (here.getBlock() instanceof SwampWaterBlock) {
			if ((up.getMaterial().isLiquid() || up.getBlock() instanceof IFluidBlock) && (up.getBlock() instanceof SwampWaterBlock == false || !((SwampWaterBlock)up.getBlock()).isUnderwaterBlock)) {
				return 1;
			}
		...
		return !here.getMaterial().isSolid() && up.getBlock() instanceof SwampWaterBlock ? 1 : this.getQuantaPercentage(world, pos) * 0.875F;
	}
	...
	public Vector3d getFlowVector(IBlockReader world, BlockPos pos) {
		...
		if (!this.isSourceBlock(world, pos) && world.getBlockState(pos.above()).getBlock() instanceof SwampWaterBlock) {
	...
	public int getQuantaValue(IBlockReader world, BlockPos pos) {
		...
		if (state.getBlock() instanceof SwampWaterBlock && ((SwampWaterBlock) state.getBlock()).isUnderwaterBlock) {
		...
		if (state.getBlock() instanceof SwampWaterBlock == false) {
			return -1;
		}
	...
	public void updateTick(World world, BlockPos pos, BlockState state, Random rand) {
		...
		if (isSourceBlock(world, pos) || !isFlowingVertically(world, pos)) {
			if (world.getBlockState(pos.below(densityDir)).getBlock() instanceof SwampWaterBlock) {
				flowMeta = 1;
			}
	...
	public AxisAlignedBB getCollisionBoundingBox(@Nonnull BlockState blockState, @Nonnull IBlockReader worldIn, @Nonnull BlockPos pos) {
		if (blockState.getBlock() instanceof SwampWaterBlock && ((SwampWaterBlock) blockState.getBlock()).isUnderwaterBlock)
	...
	public boolean canCollideCheck(BlockState state, boolean fullHit) {
		return state.getBlock() instanceof SwampWaterBlock && ((SwampWaterBlock) state.getBlock()).isUnderwaterBlock || super.canCollideCheck(state, fullHit);
	}

	@Override
	public boolean isReplaceable(IBlockReader worldIn, BlockPos pos) {
		BlockState state = worldIn.getBlockState(pos);
		return !(state.getBlock() instanceof SwampWaterBlock) || !((SwampWaterBlock) state.getBlock()).isUnderwaterBlock && super.isReplaceable(worldIn, pos);
	}

	@Override
	@OnlyIn(Dist.CLIENT)
	public void setStateMapper(AdvancedStateMap.Builder builder) {
		builder.ignore(SwampWaterBlock.LEVEL);
	}
