...
public class EventRift extends TimedEnvironmentEvent {
	public static class RiftConfiguration {
		...
		public RiftConfiguration(CompoundNBT nbt) {
	...
	protected static final DataParameter<Integer> ACTIVATION_TICKS = GenericDataManager.createKey(EventRift.class, DataSerializers.INT);
	...
	protected void initDataParameters() {
		...
		this.entityData.define(ACTIVATION_TICKS, 10, 0);
		this.entityData.define(RIFT_CONFIGURATION, new RiftConfiguration(0, 0, 0, 0, 1, false, false));
	}
	...
	public void setActive(boolean active) {
		if(!this.getWorld().isClientSide() && active && !this.isActive() && this.getActivationTicks() == 0) {
			this.setRandomConfiguration();
	...
	protected void setRandomConfiguration() {
		...
		this.dataManager.set(RIFT_CONFIGURATION, new RiftConfiguration(
				world.random.nextInt(Integer.MAX_VALUE),
				world.random.nextFloat(), world.random.nextFloat(),
				world.random.nextFloat(), world.random.nextFloat(),
				world.random.nextBoolean(), world.random.nextBoolean()));
	}
		...
		if(!this.getWorld().isClientSide()) {
			...
			if((!this.isActive() && remainingTicks < 1800 && remainingTicks > 80) || (this.isActive() && remainingTicks < 1800 && remainingTicks > 80)) {
				if(this.soundTicks-- <= 0) {
					...
					this.soundTicks = this.getWorld().random.nextInt(150) + 100;
				}
			}
		}
	}
	...
	public void saveEventData() { 
		...
		this.getRiftConfiguration().save(nbt);
	}
