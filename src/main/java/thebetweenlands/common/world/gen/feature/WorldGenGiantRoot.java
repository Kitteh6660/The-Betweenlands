...
public class WorldGenGiantRoot extends WorldGenerator {
	...
	public boolean generate(IBlockStateAccessOnly worldIn, int chunkX, int chunkZ, boolean checkChunk, Random rand, BlockPos position) {
		...
		Vector3d diff = new Vector3d(this.end.subtract(this.start));
		Vector3d dir = diff.normalize();

		Vector3d offsetDir = up.cross(dir);

		Vector3d controlPtStart = new Vector3d(this.start).subtract(dir).add(0, -16, 0);

		Vector3d controlPtEnd = new Vector3d(this.end).add(dir).add(0, -16, 0);

		List<Vector3d> pts = new ArrayList<>();

		pts.add(controlPtStart);
		pts.add(new Vector3d(this.start));

		int randPts = 5;
		for(int i = 0; i < randPts; i++) {
			Vector3d pt = new Vector3d(this.start).add(diff.scale(i / (float)randPts)).add(offsetDir.x * (rand.nextDouble() - 0.5D) * this.randXZOffsetRange * 2.0D, (rand.nextDouble() - 0.5D) * this.randYOffsetRange * 2.0D + Math.sin(i / (float)randPts * Math.PI) * this.maxArcHeight, offsetDir.z * (rand.nextDouble() - 0.5D) * this.randXZOffsetRange * 2.0D);
			pts.add(pt);
		}

		pts.add(new Vector3d(this.end));
		pts.add(controlPtEnd);

		ISpline spline = new CatmullRomSpline(pts.toArray(new Vector3d[0]));

		BlockState bark = BlockRegistry.GIANT_ROOT.defaultBlockState();
		BlockState leaves = BlockRegistry.LEAVES_WEEDWOOD_TREE.defaultBlockState().setValue(BlockLeavesBetweenlands.CHECK_DECAY, false);
		BlockState root = BlockRegistry.ROOT.defaultBlockState();
		BlockState hanger = BlockRegistry.HANGER.defaultBlockState().setValue(BlockHanger.CAN_GROW, false);
		BlockState fungus = BlockRegistry.SHELF_FUNGUS.defaultBlockState();
				...
				for(int yo = -radius; yo <= radius; yo++) {
					for(int zo = -radius; zo <= radius; zo++) {
						...
						if(!this.genLeaves) {
							...
							if(this.isInBounds(genPos) && foliageRand.nextInt(60) == 0 && yo >= radius - 2 && xo*xo+yo*yo+zo*zo <= (radius-1)*(radius-1) && !worldIn.isEmptyBlock(genPos) && worldIn.isEmptyBlock(genPos.above()) && worldIn.isEmptyBlock(genPos.above(2))) {
								int maxRootHeight = 2 + foliageRand.nextInt(3);
								for(int r = 0; r < maxRootHeight; r++) {
									BlockPos rootPos = genPos.add(0, 1 + r, 0);
									if(worldIn.isEmptyBlock(rootPos)) {
							...
							if(this.isInBounds(genPos) && foliageRand.nextInt(10) == 0 && yo <= radius - 2 && xo*xo+yo*yo+zo*zo <= (radius-1)*(radius-1) && !worldIn.isEmptyBlock(genPos) && worldIn.isEmptyBlock(genPos.below()) && worldIn.isEmptyBlock(genPos.below(2))) {
								int maxHangersHeight = 3 + foliageRand.nextInt(16);
								for(int r = 0; r < maxHangersHeight; r++) {
									BlockPos hangerPos = genPos.add(0, -1 - r, 0);
									if(worldIn.isEmptyBlock(hangerPos)) {
							...
							if(this.isInBounds(genPos) && foliageRand.nextInt(10) == 0 && xo*xo+yo*yo+zo*zo > (radius-1)*(radius-1) && worldIn.isEmptyBlock(genPos)) {
								...
								for(Direction facing : dirs) {
									...
									if((xo+facing.getStepX())*(xo+facing.getStepX())+(yo+facing.getStepY())*(yo+facing.getStepY())+(zo+facing.getStepZ())*(zo+facing.getStepZ()) <= (radius-1)*(radius-1)) {
										worldIn.setBlockState(genPos, BlockRegistry.MOSS.defaultBlockState().setValue(BlockMoss.FACING, facing.getOpposite()), 2);
									}
								}
							}
						}
