...
public class RubberTreeFeature extends Feature<BaseTreeFeatureConfig>
{
	...
	public RubberTreeFeature(Codec<BaseTreeFeatureConfig> codecIn) {
		super(codecIn);
	}
	...
	public final boolean place(ISeedReader world, ChunkGenerator chunkGen, Random rand, BlockPos position, BaseTreeFeatureConfig config) {
		...
		this.log = BlockRegistry.LOG_RUBBER.defaultBlockState().setValue(BlockRubberLog.NATURAL, true);
		this.leaves = BlockRegistry.LEAVES_RUBBER_TREE.defaultBlockState().setValue(BlockLeavesBetweenlands.CHECK_DECAY, false);

		for (int xx = x - maxRadius; xx <= x + maxRadius; xx++)
			for (int zz = z - maxRadius; zz <= z + maxRadius; zz++)
				for (int yy = y + 2; yy < y + height; yy++)
					if (!world.isEmptyBlock(checkPos.set(xx, yy, zz)) && !world.getBlockState(checkPos.set(xx, yy, zz)).getBlock().canBeReplacedByLogs(world, checkPos.set(xx, yy, zz)))
						return false;
