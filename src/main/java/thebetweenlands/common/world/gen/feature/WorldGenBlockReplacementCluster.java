...
public class WorldGenBlockReplacementCluster extends WorldGenerator {
	...
	public WorldGenBlockReplacementCluster(BlockState blockState, int offset, int attempts, Predicate<BlockState> matcher) {
	...
	public WorldGenBlockReplacementCluster(BlockState blockState, Predicate<BlockState> matcher) {
		...
		for (int i = 0; i < this.attempts; ++i) {
			...
			if (worldIn.isBlockLoaded(blockpos)) {
				...
				if (this.matcher.apply(state)) {
					BlockState setState = this.blockState;
					if (this.inheritProperties) {
						...
						for (Entry<IProperty<?>, Comparable<?>> property : properties.entrySet()) {
							...
							if (targetProperty != null && sourceProperty.getValueClass() == targetProperty.getValueClass()) {
								setState = setState.setValue(targetProperty, (Comparable) property.getValue());
							}
						}
					}
