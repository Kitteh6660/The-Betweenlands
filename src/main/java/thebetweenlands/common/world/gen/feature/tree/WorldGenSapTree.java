...
public class WorldGenSapTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		this.logX = BlockRegistry.LOG_SAP.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.X);
		this.logY = BlockRegistry.LOG_SAP.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y);
		this.logZ = BlockRegistry.LOG_SAP.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Z);
		this.roots = BlockRegistry.LOG_SAP.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.NONE);
		this.leaves = BlockRegistry.LEAVES_SAP_TREE.defaultBlockState().setValue(BlockLeavesBetweenlands.CHECK_DECAY, false);

		for (int xx = - maxRadius; xx <= maxRadius; xx++)
			for (int zz = - maxRadius; zz <= maxRadius; zz++)
				for (int yy = 2; yy < height; yy++)
					if (!world.isEmptyBlock(pos.offset(xx, yy, zz)) && !world.getBlockState(pos.offset(xx, yy, zz)).getBlock().isReplaceable(world, pos.offset(xx, yy, zz)))
						return false;

		for (int yy = 0; yy < height; ++yy) {
			world.setBlockState(pos.offset(0, yy, 0), logY, 2);
	...
	private void createBranch(World world, Random rand, BlockPos pos, int dir, int branchLength) {
		...
		for (int i = 0; i <= branchLength; ++i) {
			...
			if (dir == 1) {
				world.setBlockState(pos.offset(i, y, 0), branchBend ? logY : logX, 2);
				if(i == branchLength) {
			...
			if (dir == 3) {
				world.setBlockState(pos.offset(0, y, i), branchBend ? logY : logZ, 2);
				if(i == branchLength) {
			...
			if (dir == 5) {
				world.setBlockState(pos.offset(i - 1, y, i - 1),branchBend ? logY : logX, 2);
				if(i == branchLength) {
			...
			if (dir == 7) {
				world.setBlockState(pos.offset(- i + 1, y, i - 1), branchBend ? logY : logZ, 2);
				if(i == branchLength) {
					createLeaves(world, pos.offset(- i + 1, y, i - 1), false);
				}
			}
	...
	private void createLeaves(World world, BlockPos pos, boolean top) {
		world.setBlockState(pos.offset(0, 0, 1), leaves, 2);
		world.setBlockState(pos.offset(0, 0, - 1), leaves, 2);
		world.setBlockState(pos.offset(1, 0, 0), leaves, 2);
		world.setBlockState(pos.offset(- 1, 0, 0), leaves, 2);

		if(top)
			world.setBlockState(pos.offset(0, 1, 0), leaves, 2);
		else
			world.setBlockState(pos.offset(0, - 1, 0), leaves, 2);
	}

	private void createRoots(World world, BlockPos pos) {
		world.setBlockState(pos.offset(0, 0, 1), roots, 2);
		world.setBlockState(pos.offset(0, 0, - 1), roots, 2);
		world.setBlockState(pos.offset(1, 0, 0), roots, 2);
		world.setBlockState(pos.offset(- 1, 0, 0), roots, 2);
	}
