...
public class NibbletwigTreeFeature extends WorldGenHelper {
	@Override
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if(!this.rotatedCubeCantReplace(world, x, y + 2, z, -2, 0, -2, 5, height, 5, 0)) {
			...
			for(int i = 0; i <= height; i++) {
				...
				if(i == bend1 || i == bend2) {
					Direction randOffset = Direction.Plane.HORIZONTAL[rand.nextInt(Direction.Plane.HORIZONTAL.length)];
			...
			for(int i = 0; i < droops; i++) {
				...
				if(Math.abs(droopXO) + Math.abs(droopZO) > 1 && !generatedDroops[droopIndex] &&
					...
					Direction ivyOffset = Direction.Plane.HORIZONTAL[rand.nextInt(Direction.Plane.HORIZONTAL.length)];
