...
public class HearthgroveTreeFeature extends Feature<BaseTreeFeatureConfig> {
	
	public HearthgroveTreeFeature(Codec<BaseTreeFeatureConfig> codec) {
		super(codec);
	}
					...
					if(!generatedRoots[rootIndex] && !generatedRoots[rootIndexN] && !generatedRoots[rootIndexS] &&
						...
						for(int j = rootHeight; j >= -4; j--) {
							if(j < rootHeight && rand.nextInt(3) == 0) {
								Direction offset = Direction.Plane.HORIZONTAL[rand.nextInt(Direction.Plane.HORIZONTAL.length)];
