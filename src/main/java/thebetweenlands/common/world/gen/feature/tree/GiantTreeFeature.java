...
public class GiantTreeFeature extends GiantTreeTrunkFeature {
			...
			for (int z = blockZ - maxRadius; z <= blockZ + maxRadius; z++) {
				for (int y = blockY; y < blockY + maxRadius; y++) {
					...
					if (dist < maxRadius - 1 && rand.nextInt(4) == 0 && y > blockY) {
						if (world.getBlockState(new BlockPos(x, y, z)).getBlock() != BlockRegistry.WEEDWOOD_LOG) {
							this.setBlockAndNotifyAdequately(world, new BlockPos(x, y, z), BlockRegistry.WEEDWOOD_LOG.defaultBlockState());
						}
					}
					if (dist <= maxRadius) {
						if (world.getBlockState(new BlockPos(x, y, z)).getBlock() != BlockRegistry.WEEDWOOD_LOG && world.getBlockState(new BlockPos (x, y, z)) != BARK) {
