...
public class WorldGenWeedwoodBush extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		do {
			...
			if (!(block.isLeaves(blockState, world, offsetPos) || world.isEmptyBlock(offsetPos))) {
				break;
			}
			--y;
		} while (y > 0);
		...
		this.setBlockAndNotifyAdequately(world, pos, BlockRegistry.WEEDWOOD_BUSH.defaultBlockState());
		...
		for (int by = y; by < y + height; ++by) {
			...
			for (int bx = x - radius; bx <= x + radius; ++bx) {
				...
				for (int bz = z - radius; bz <= z + radius; ++bz) {
					...
					BlockState checkBlockState = world.getBlockState(offsetPos);

					if (checkBlockState.getBlock().canBeReplacedByLeaves(checkBlockState, world, offsetPos) && rand.nextInt((int)((xo*xo+zo*zo)*1.25+1)) < 2) {
						BlockState blockStateBelow = world.getBlockState(offsetPos.setPos(bx, by - 1, bz));
						if(blockStateBelow.getBlock() == BlockRegistry.WEEDWOOD_BUSH || SurfaceType.GRASS_AND_DIRT.matches(blockStateBelow)) {
							this.setBlockAndNotifyAdequately(world, new BlockPos(bx, by, bz), BlockRegistry.WEEDWOOD_BUSH.defaultBlockState());
						}
					}
				}
			}
