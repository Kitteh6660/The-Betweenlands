...
public class WorldGenBigBulbCappedMushroom extends WorldGenerator {
	...
	public void generateHead(World world, BlockPos pos){
		...
		world.setBlockState(pos.offset(0, -1, 0), head, 2);
		int startY = pos.getY();
		for(int yy = startY; yy >= startY - 4; yy--){
			pos = new BlockPos(pos.getX(), yy, pos.getZ());
			world.setBlockState(pos.offset(1, 0, 0), head, 2);
			world.setBlockState(pos.offset(-1, 0, 0), head, 2);
			world.setBlockState(pos.offset(0, 0, 1), head, 2);
			world.setBlockState(pos.offset(0, 0, -1), head, 2);
			world.setBlockState(pos.offset(1, 0, -1), head, 2);
			world.setBlockState(pos.offset(-1, 0, -1), head, 2);
			world.setBlockState(pos.offset(1, 0, 1), head, 2);
			world.setBlockState(pos.offset(-1, 0, 1), head, 2);
			if(yy >= startY - 3 && yy <= startY - 1){
				world.setBlockState(pos.offset(-2, 0, 0), head, 2);
				world.setBlockState(pos.offset(2, 0, 0), head, 2);
				world.setBlockState(pos.offset(0, 0, -2), head, 2);
				world.setBlockState(pos.offset(0, 0, 2), head, 2);
				world.setBlockState(pos.offset(-2, 0, -1), head, 2);
				world.setBlockState(pos.offset(-2, 0, 1), head, 2);
				world.setBlockState(pos.offset(2, 0, -1), head, 2);
				world.setBlockState(pos.offset(2, 0, 1), head, 2);
				world.setBlockState(pos.offset(-1, 0, 2), head, 2);
				world.setBlockState(pos.offset(1, 0, 2), head, 2);
				world.setBlockState(pos.offset(-1, 0, -2), head, 2);
				world.setBlockState(pos.offset(1, 0, -2), head, 2);
			}
	...
	public boolean generate(World world, Random rand, BlockPos position) {
		this.head = BlockRegistry.BULB_CAPPED_MUSHROOM_CAP.defaultBlockState();
		this.stalk = BlockRegistry.BULB_CAPPED_MUSHROOM_STALK.defaultBlockState().setValue(RotatedPillarBlock.AXIS, EnumAxis.Y);
		...
		for (int xx = x - maxRadius; xx <= x + maxRadius; xx++)
			for (int zz = z - maxRadius; zz <= z + maxRadius; zz++)
				for (int yy = y + 2; yy < y + height; yy++)
					if (!world.isEmptyBlock(checkPos.setPos(xx, yy, zz)))
						return false;
