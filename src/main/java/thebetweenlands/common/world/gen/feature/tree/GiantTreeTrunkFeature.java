...
public abstract class GiantTreeTrunkFeature extends TreeFeature {
	
	public GiantTreeTrunkFeature(Codec<BaseTreeFeatureConfig> codec) {
		super(codec);
	}
	...
	public static BlockState BARK = BlockRegistry.WEEDWOOD_LOG.get().defaultBlockState().setValue(RotatedPillarBlock.AXIS, Direction.Axis.Y);
	public static BlockState WOOD = BlockRegistry.WEEDWOOD.get().defaultBlockState();
	public static BlockState LEAVES = BlockRegistry.LEAVES_WEEDWOOD_TREE.get().defaultBlockState().setValue(BlockLeavesBetweenlands.CHECK_DECAY, false);
	public static BlockState IVY = BlockRegistry.POISON_IVY.get().defaultBlockState();
	public static BlockState HANGER = BlockRegistry.HANGER.get().defaultBlockState();
	...
	@Override
	public boolean generate(IWorldGenerationReader world, Random rand, BlockPos pos) {
		...
		for (int dy = 0; dy < height; dy++) {
			...
			for (int dx = -radius; dx <= radius; dx++) {
				for (int dz = -radius; dz <= radius; dz++) {
					...
					switch (state) {
					case 0:
						if (world.getBlockState(new BlockPos(blockX + dx, y - 1, blockZ + dz)).getBlock() == BlockRegistry.WEEDWOOD_LOG) {
