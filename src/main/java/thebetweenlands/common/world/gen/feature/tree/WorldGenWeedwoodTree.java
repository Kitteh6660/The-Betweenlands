...
public class WorldGenWeedwoodTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		this.logX = BlockRegistry.LOG_WEEDWOOD.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.X);
		this.logY = BlockRegistry.LOG_WEEDWOOD.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y);
		this.logZ = BlockRegistry.LOG_WEEDWOOD.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Z);
		this.bark = BlockRegistry.LOG_WEEDWOOD.defaultBlockState().setValue(BlockLog.LOG_AXIS, BlockLog.EnumAxis.NONE);
		this.wood = BlockRegistry.WEEDWOOD.defaultBlockState();
		this.leaves = BlockRegistry.LEAVES_WEEDWOOD_TREE.defaultBlockState().setValue(BlockLeavesBetweenlands.CHECK_DECAY, false);
			...
			for (int i = radius * -1; i <= radius; ++i)
				for (int j = radius * -1; j <= radius; ++j) {
					...
					if (Math.round(Math.sqrt(dSq)) < radius && yy <= 1 + height - 2)
						world.setBlockState(pos.offset(i, yy, j), wood, 2);
					if (Math.round(Math.sqrt(dSq)) == radius && yy == 0 || Math.round(Math.sqrt(dSq)) == radius && yy <= height - 1)
						world.setBlockState(pos.offset(i, yy, j), bark, 2);
				}
	...
	private void createSmallBranch(World world, Random rand, BlockPos pos, int dir, int branchLength) {
		...
		for (int i = 0; i <= branchLength; ++i) {
			...
			switch (dir) {
			case 1:
				world.setBlockState(pos.east(i).above(y), branchBend ? logY : logX, 2);
				break;

			case 2:
				world.setBlockState(pos.west(i).above(y),  branchBend ? logY : logX, 2);
				break;

			case 3:
				world.setBlockState(pos.south(i).above(y),  branchBend ? logY : logZ, 2);
				break;

			case 4:
				world.setBlockState(pos.north(i).above(y),  branchBend ? logY : logZ, 2);
				break;

			case 5:
				world.setBlockState(pos.east(i).above(y).south(i), branchBend ? logY : logX, 2);
				break;

			case 6:
				world.setBlockState(pos.west(i).above(y).north(i), branchBend ? logY : logX, 2);
				break;

			case 7:
				world.setBlockState(pos.west(i).above(y).south(i), branchBend ? logY : logZ, 2);
				break;

			case 8:
				world.setBlockState(pos.east(i).above(y).north(i), branchBend ? logY : logZ, 2);
				break;
			}
		}
	}
				...
				for (int y1 = 0; y1 < maxRadius; y1++) {
					...
					if (Math.round(Math.sqrt(dSq)) < maxRadius - 1 && y1 > 0)
						if (!isALog(world.getBlockState(pos.offset(x1, y1, z1))))
							world.setBlockState(pos.offset(x1, y1, z1), logY);
				}
				...
				for (int y1 = 0; y1 < maxRadius; y1++) {
					...
					if (Math.round(Math.sqrt(dSq)) <= maxRadius)
						if (!isALog(world.getBlockState(pos.offset(x1, y1, z1))) && rand.nextInt(5) != 0)
							world.setBlockState(pos.offset(x1, y1, z1), leaves, 2);
						...
						if (world.getBlockState(pos.offset(x1, y1, z1)) == leaves)
							for (int i = 1; i < 1 + rand.nextInt(3); i++)
								if (!isALog(world.getBlockState(pos.offset(x1, y1 - i, z1))))
									world.setBlockState(pos.offset(x1, y1 - i, z1), leaves, 2);
				}
	}
		...
		for (int i = 0; i <= branchLength; ++i) {
			...
			case 1:
				...
				if (!root) {
					...
					world.setBlockState(pos.east(i).below(y), bark, 2);
					world.setBlockState(pos.east(i).below(y - 1), bark, 2);
				}
				break;
			...
			case 2:
				if (!root) {
					...
					world.setBlockState(pos.west(i).below(y), bark, 2);
					world.setBlockState(pos.west(i).below(y - 1), bark, 2);
				}
				break;
			...
			case 3:
				if (!root) {
					...
					world.setBlockState(pos.south(i).below(y), bark, 2);
					world.setBlockState(pos.south(i).below(y - 1), bark, 2);
				}
				break;
			...
			case 4:
				if (!root) {
					...
					world.setBlockState(pos.north(i).below(y), bark, 2);
					world.setBlockState(pos.north(i).below(y - 1), bark, 2);
				}
				break;

			case 5:
				if (!root) {
					world.setBlockState(pos.east(i - 1).above(y).south(i - 1), branchBend ? logY : logX, 2);
					if (i <= branchLength && ivy)
						addVines(world, rand, pos.east(i - 1).above(y - 1).south(i - 1), Direction.EAST);
				}
				else {
					world.setBlockState(pos.east(i - 1).below(y).south(i - 1), bark, 2);
					world.setBlockState(pos.east(i - 1).below(y - 1).south(i - 1), bark, 2);
				}
				break;

			case 6:
				if (!root) {
					world.setBlockState(pos.west(i - 1).above(y).north(i - 1), branchBend ? logY : logX, 2);
					if (i <= branchLength && ivy)
						addVines(world, rand, pos.west(i - 1).above(y - 1).north(i - 1), Direction.WEST);
				}
				else {
					world.setBlockState(pos.west(i - 1).below(y).north(i - 1), bark, 2);
					world.setBlockState(pos.west(i - 1).below(y - 1).north(i - 1), bark, 2);
				}
				break;

			case 7:
				if (!root) {
					world.setBlockState(pos.west(i - 1).above(y).south(i - 1), branchBend ? logY : logZ, 2);
					if (i <= branchLength && ivy)
						addVines(world, rand, pos.west(i - 1).above(y - 1).south(i - 1), Direction.SOUTH);
				}
				else {
					world.setBlockState(pos.west(i - 1).below(y).south(i - 1), bark, 2);
					world.setBlockState(pos.west(i - 1).below(y - 1).south(i - 1), bark, 2);
				}
				break;

			case 8:
				if (!root) {
					world.setBlockState(pos.east(i - 1).above(y).north(i - 1), branchBend ? logY : logZ, 2);
					if (i <= branchLength && ivy)
						addVines(world, rand, pos.east(i - 1).above(y - 1).north(i - 1), Direction.NORTH);
				}
				else {
					world.setBlockState(pos.east(i - 1).below(y).north(i - 1), bark, 2);
					world.setBlockState(pos.east(i - 1).below(y - 1).north(i - 1), bark, 2);
				}
				break;
			}
		}
	...
	private void addVines(World world, Random rand, BlockPos pos, Direction facing) {
		if (rand.nextInt(4) != 0) {
			int length = rand.nextInt(4) + 4;
			for (int yy = 0; yy < length; ++yy)
				if (world.isEmptyBlock(pos.below(yy)))
					world.setBlockState(pos.below(yy), ivy.setValue(BlockVineBL.getPropertyFor(facing.getOpposite()), Boolean.valueOf(true)), 2);
				else
					break;
		}
	}
