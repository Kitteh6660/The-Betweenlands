...
public class GiantTreeDeadFeature extends GiantTreeTrunkFeature {
	...
	public void addVineToRandomBlockSide(World world, Random rand, int x, int y, int z) {
		...
		if (world.getBlockState(new BlockPos(x + direction.getStepX(), y, z + direction.getStepZ())).getMaterial().isReplaceable()) {
			world.setBlock(new BlockPos(x + direction.getStepX(), y, z + direction.getStepZ()), IVY.setValue(BlockVineBL.getPropertyForFace(direction.getOpposite()), Boolean.valueOf(true)), 2);
		}
	}
